...
module.exports = async function login () {
	...
	return new Promise(async function (resolve, reject) {
		...
		if (loginOptions.cookie) {
			...
			let response = await self._setup.request.request("https://auth.roblox.com/v2/login", {
				method: "POST",
				json: loginOptions
			});
			...
			if (body && body.errors && body.errors[0] && body.errors[0].code === 2) {
				...
				self._setup.captchaManager.solveCaptcha("https://www.roblox.com/newlogin").then(async (captchaResponse) => {
					let verifiedLogin = await CaptchaManager.verifyCaptchaLogin(captchaResponse, loginOptions.cvalue, self._setup);
